...
import com.example.appcontentprovider.database.MyContentProvider.Companion.URI_NOTES
import com.example.appcontentprovider.database.NotesDatabaseHelper.Companion.DESCRIPTION_NOTES
import com.example.appcontentprovider.database.NotesDatabaseHelper.Companion.TITLES_NOTES
...
class NotesDetailFragment: DialogFragment(), DialogInterface.OnClickListener {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if(arguments != null && arguments?.getLong(EXTRA_ID) != 0L){
            ...
            if(cursor?.moveToNext() as Boolean){
                newNote = false
                noteEditTitle.setText(cursor.getString(cursor.getColumnIndex(TITLES_NOTES)))
    ...
    override fun onClick(dialog: DialogInterface?, which: Int) {
        ...
        values.put(TITLES_NOTES, noteEditTitle.text.toString())
